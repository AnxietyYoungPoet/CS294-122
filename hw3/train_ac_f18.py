...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            # YOUR_HW2 CODE_HERE
            sy_mean = build_mlp(
                self.sy_ob_no, self.ac_dim, 'policy', self.n_layers, self.size)
    ...
    def build_computation_graph(self):
        ...
        self.critic_prediction = tf.squeeze(build_mlp(
            self.sy_ob_no, 1, "nn_critic", n_layers=self.n_layers, size=self.size))
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        """
            ...
            let sum_of_path_lengths be the sum of the lengths of the paths sampled froma
        ...
        # YOUR CODE HERE
        v_st = self.sess.run(self.critic_prediction, {self.sy_ob_no: ob_no})
        v_stp1 = self.sess.run(self.critic_prediction, {self.sy_ob_no: next_ob_no})
        adv_n = re_n + self.gamma * (1 - terminal_n) * v_stp1 - v_st
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            v_stp1 = self.sess.run(self.critic_prediction, {self.sy_ob_no: next_ob_no})
            target_n = re_n + self.gamma * (1 - terminal_n) * v_stp1
